...
Sentinel uses a permanent cache because types are immutable at runtime:
...
- **Dependency injection**: Adds complexity for testing, but `Reset()` provides test isolation (with `-tags testing`)
