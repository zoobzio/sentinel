...
Sentinel extracts metadata at two levels. [Type metadata](../3.reference/2.types.md#metadata) captures the struct as a whole—its name, package, and the graph of types it references. [Field metadata](../3.reference/2.types.md#fieldmetadata) captures each exported field—its name, type, kind, and struct tags.
...
Struct fields that reference other structs create [relationships](../3.reference/2.types.md#typerelationship). Sentinel categorizes these into [four kinds](../3.reference/2.types.md#relationship-kinds):
...
Each field has a [kind](../3.reference/2.types.md#fieldkind) that categorizes its type for conditional logic: `scalar`, `pointer`, `slice`, `struct`, `map`, or `interface`. This lets you branch on the shape of a field without parsing type strings.
...
- [API Reference](../3.reference/1.api.md) — complete type and function documentation
